...
- [x] Define staging/fixture data for ABS in E2E — done: `scripts/make-fixture.sh` (generates the single-book/single-track fixture WAV) + `scripts/seed-abs.sh` (seeds ABS: library, forced scan, end user + streamer API key, initial listening position) together fully define the fixture data E2E-01..10 need, and the suite runs green against it. A richer fixture set (multi-track book for track-boundary seeks, multiple library items for search/pagination) isn't required by any current §5 scenario; captured as follow-up for when such scenarios land: [#13](https://github.com/Xexanos/ratatoskr-e2e/issues/13)
...
- [x] Provision E2E secrets/fixtures beyond ABS content: a known server TLS cert + fingerprint (E2E-01 trust-on-first-use) and ABS user credentials + the low-privilege streamer account (E2E-02) — resolved without provisioning any long-lived secret: the server entrypoint generates a fresh self-signed cert per run, and `run-e2e.sh` reads its SHA-256 fingerprint at runtime via openssl and passes it to the Maestro flow as `CERT_FP`, so the TOFU assertion checks the real per-run fingerprint; `seed-abs.sh` creates the end user and the stream-only user + API key inside the throwaway per-run ABS instance, with hardcoded non-secret values since nothing outlives the run. The only remaining repo inputs — `E2E_DISPATCH_TOKEN`, `SERVER_PROMOTE_TOKEN`, `APP_PROMOTE_TOKEN` — are CI plumbing tokens, already provisioned and out of scope for this point (see [#11](https://github.com/Xexanos/ratatoskr-e2e/issues/11))
